...
void loop() {
  if (digitalRead(BOTON) != ultimoEstado) { // si el estado actual no es igual al anterior
    if (digitalRead(BOTON) == HIGH) { // si el boton esta presionado
      ledIndex++; // aumenta el valor del index del led encendido
      if (ledIndex >= 4) { // si es 4 reiniciar el contador
        ledIndex = 0;
      }
      apagarTodos();
      Serial.println(ledIndex);
      digitalWrite(ledIndex + 2, HIGH); // enciende el led correspondiente
    }
